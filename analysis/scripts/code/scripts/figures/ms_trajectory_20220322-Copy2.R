...
# pdf(file=file.path(proj_root, "figures", "obsolete", "pooled_fsk.mesenchymal.monocle.hoxc5_early_fibroblast_to_dermal_papillia.trajectory_DE.20220308_no_adipocytes.pdf"), width=10, height=6)
...
# pdf(file=file.path(proj_root, "figures", "obsolete", "pooled_org.mesenchymal.monocle.hoxc5_early_fibroblast_to_dermal_papillia.trajectory_DE.20220308_no_adipocytes.pdf"), width=10, height=6)
# grid.draw(rectGrob(gp=gpar(fill = "white", lwd = 0)))
...
# 20220322 analysis
### FSK subset ####
pooled_cds <- readRDS(file.path(data_root, '20211022_final_figures', 'monocle', "pooled_fsk_org.mesenchymal.monocle.20220308.downsampled.rds"))
...
org_cds <- readRDS(file.path(data_root, '20211022_final_figures', 'monocle', "pooled_org.mesenchymal.monocle.20220308.downsampled.rds"))


pooled_sub <- row.names(subset(pData(pooled_cds), joint_annot != "Adipocytes"))
fsk_sub <- row.names(subset(pData(fsk_cds), joint_annot != "Adipocytes"))
org_sub <- row.names(subset(pData(org_cds), joint_annot != "Adipocytes"))

pooled_cds <- pooled_cds[,pooled_sub]
fsk_cds <- fsk_cds[,fsk_sub]
org_cds <- org_cds[,org_sub]
...
# print(get_principal_node(ms_cds_list1[["fsk"]], "joint_annot", "HOXC5+ early fibroblast", node_type="any"))
# print(get_principal_node(ms_cds_list1[["fsk"]], "joint_annot", "Dermal papillia", node_type="leaf"))
# print(get_principal_node(ms_cds_list1[["fsk"]], "joint_annot", "PEAR1+ fibroblast", node_type="leaf"))
...
fsk_ms_cds2 <- choose_graph_segments(
  ms_cds_list1[["fsk"]],
  starting_pr_node = get_principal_node(ms_cds_list1[["fsk"]], "joint_annot", "HOXC5+ early fibroblast", node_type="any"),
  ending_pr_nodes = c(get_principal_node(ms_cds_list1[["fsk"]], "joint_annot", "Dermal papillia", node_type="leaf"), 
                      get_principal_node(ms_cds_list1[["fsk"]], "joint_annot", "PEAR1+ fibroblast", node_type="leaf")),
  return_list=FALSE
)

unique(fsk_ms_cds2$joint_annot)
...
# # source("~/src/github/Teichlab/sctkr/R/MonocleUtils.R")
# # p0_pdf <- file.path(proj_root, "figures", "final", "pooled_fsk.mesenchymal_subset.monocle.20220329_no_adipocytes.pdf")
# p0_pdf <- file.path(proj_root, "figures", "final", "pooled_fsk.mesenchymal_subset.monocle.20220823_no_adipocytes.pdf")
...
# fsk_p1_deg <- graph_test(fsk_p1_cds2, neighbor_graph = "principal_graph", cores = 20)
...
# p1_pdf <- file.path(proj_root, "figures", "final", "pooled_fsk.mesenchymal.monocle.hoxc5_early_fibroblast_to_dermal_papillia.trajectory_DE.20220711_no_adipocytes.pdf")
...
# p2_pdf <- file.path(proj_root, "figures", "final", "pooled_fsk.mesenchymal.monocle.hoxc5_early_fibroblast_to_pear1_fibroblast.trajectory_DE.20220823_no_adipocytes.pdf")
...
# fsk_p3_cds2 <- fsk_p3_cds2[, colData(fsk_p3_cds2)$joint_annot %in% c("HOXC5+ early fibroblast", "WNT2+ fibroblast", "PEAR1+ fibroblast")]
...
# fsk_p3_deg <- graph_test(fsk_p3_cds2, neighbor_graph = "principal_graph", cores = 20)
...
# )
# # maybe the following should be in "final"
# p3_pdf <- file.path(proj_root, "figures", "final", "pooled_fsk.mesenchymal.monocle.hoxc5_early_fibroblast_to_PEAR1+ fibroblast.trajectory_DE.20220711_no_adipocytes.pdf")
